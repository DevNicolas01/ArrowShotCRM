...
function WidgetCard({
  ...
  return (
    ...
    >
      <div className={`flex min-w-0 items-center gap-2 rounded-t-2xl px-5 py-3.5 ${styles.headerBg ?? ''}`}>
        <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${styles.iconBg}`}>{icon}</div>
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1 px-5 pb-4 pt-1">{children}</div>
      {footer && <div className="min-w-0 border-t border-slate-100 px-5 py-2.5">{footer}</div>}
    </div>
  )
}
...
export function DashboardPage() {
  ...
  return (
    <div className="-m-3 min-h-full bg-slate-100 p-3 sm:-m-5 sm:p-5">
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
            <div className="flex items-center gap-2 rounded-t-2xl px-5 py-3.5">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-violet-500">
                <CalendarDays size={14} className="text-white" />
              </div>
              <p className="text-sm font-semibold text-slate-700">Próximas publicações</p>
              <span className="ml-auto text-xs font-medium text-slate-400">{buckets.nextPublications.length}</span>
            </div>
            <div className="flex flex-1 flex-col gap-1 px-5 pb-4 pt-1">
          ...
          <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
                    ...
                    {clientSummary.map(({ client, health, service, ownerName, nextTask }, index) => {
                      ...
                      return (
                        <tr
                          ...
                          className={`cursor-pointer border-t border-slate-50 text-slate-700 transition-colors duration-150 ease-in-out hover:bg-blue-50 ${
                        ...
                        >
                          ...
                          <td className={`py-2 pr-2 text-xs ${overdueTask ? 'font-bold text-red-600' : 'text-slate-400'}`}>
